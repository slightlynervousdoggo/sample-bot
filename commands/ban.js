...
module.exports = {
  ...
  async execute(message, args) {
    if (!message.member.roles.some(r => ROLESWITHBANPERMISSION.includes(r.id))) return message.reply("Sorry, you don't have permissions to use this command!")

    const member = message.mentions.members.first() || message.guild.members.get(args[0]);

    // Check if the user is in the server
    if (!member) return message.reply("Member is not in this server")
